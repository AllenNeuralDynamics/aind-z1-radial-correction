...
def radial_correction(tile_data, corner_shift=5.5, frac_cutoff=0.5):
    ...
    grid = np.array(np.meshgrid(np.arange(pixels), np.arange(pixels), indexing='ij'))
    coords = (grid - pixels//2).astype(np.float32)
    
    r = np.sqrt((coords**2).sum(0))
    angle = np.arctan2(coords[0], coords[1])
    rmax = r.max()
    r_piece = r + (r > cutoff) * (r - cutoff) * corner_shift / (rmax - cutoff) # piecewise linear
    

    coords[0], coords[1] = r_piece*np.sin(angle), r_piece*np.cos(angle)
    coords = np.array(coords[:,edge:-edge, edge:-edge] + pixels//2, np.float32)
    ...
    warp_coords = np.array(np.meshgrid(*arange_list, indexing='ij')) #this line is problematic - using too much memory
    # chunksize = 256
...
def radial_correction_2d(tile_data, corner_shift=5.5, frac_cutoff=0.5):
    ...
    for z in range(tile_data.shape[0]):
        img[z] = map_coordinates(tile_data[z], warp_coords, order=1, mode='constant')
    return img
...
def apply_corr_to_zarr_tile(zarr_file_loc, corner_shift=5.5, frac_cutoff=0.5):
    ...
    if z_size < 400:
        corrected_tile = radial_correction(tile[0,0,...].compute(), corner_shift, frac_cutoff)
    else: 
        corrected_tile = radial_correction_2d(tile[0,0,...].compute(), corner_shift, frac_cutoff)
...
def correct_and_save_tile(dataset_loc, output_path, resolution_zyx):
   ...
   # out_group = root_group.create_group(tilename, overwrite=True) 
    ...
    corrected_tile = apply_corr_to_zarr_tile(dataset_loc+'/0', corner_shift, frac_cutoff)
    #make dask array
    ...
    with performance_report(filename=f"/results/{tilename}-dask-report.html"):
        ...
        zarr.save_array( zarr_loc, corrected_tile) #slow... consider doing with dask
