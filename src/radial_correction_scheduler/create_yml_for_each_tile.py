...
def _load_manifest(DATA_FOLDER):
    ...
    manifest_path = list(Path(DATA_FOLDER).glob("derived/processing_manifest.json"))
    ...
    try:
        with open(manifest_path[0], 'r') as f:
            ...
            print(f"Loaded manifest with channels: {manifest.get('spot_channels', [])}")
...
def write_output_yml(dataset_loc, yml_file_name, input_path, resolution_zyx = (1, 0.256, 0.256)):
    ...
    yml_dict = {"input_path": str(input_path), "output_path": str(dataset_loc), "resolution_zyx": list(resolution_zyx)}
...
def write_yml_for_worker(dataset_loc, output_path):
    ...
    to_save_loc = f'{output_path}/radial_correction/{dataset_loc.parent.stem}.zarr'


    yml_file_name = f'/results/{dataset_loc.parent.stem}_to_do_radial_correction.yml'

    write_output_yml(to_save_loc, yml_file_name, to_read_loc, zyx_voxel_size_um)
...
if __name__ == "__main__":
    ...
    try: #run from pipeline
        ...
        if len(xml_loc) >0:
            local_xml_loc = [xml_loc_str for xml_loc_str in xml_loc if 'remote' not in xml_loc_str]
        ...
        if len(xml_loc) == 0 :
            xml_loc = Path(dataset_loc).parent.joinpath('stitching_single_channel.xml').as_posix()
        else:
            local_xml_loc = [xml_loc_str for xml_loc_str in xml_loc if 'remote' not in xml_loc_str]
            xml_loc = str(local_xml_loc[0])
